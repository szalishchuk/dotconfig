...
# DEFAULT CONFIGS
ICON_TOTAL = "  "
DATABASE_PATH = "./pomodoro.db"

# CUSTOM CONFIGS
config = configparser.ConfigParser()
config.read('./pomodoro.conf')
...
def kill_another_instances():
    ...
    os.system("pgrep pomodoro > pomodoro.pid")
    filePids = open("pomodoro.pid", "r")
    for pid in filePids:
        pid = int(pid)
        if pid != int(os.getpid()):
            os.system("kill -9 " + str(pid))
    filePids.close()
    os.system("rm pomodoro.pid")
...
def writeOutput(message):
    ...
    try:
        print(message)
        fileOutput = open("./pomodoro.output", "w")
...
def recharge_polybar():
    ...
    os.system("polybar-msg hook pomodoro 1 &> /dev/null")
